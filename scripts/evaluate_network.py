...
def evaluate_torch(model,
    ...
    for data in val_dataset:
        ...
        if data['frame_id0'][0] % skip == 4:
            ...
            if len(
                    set([
                        frames[0]['scene_id0'][0], frames[1]['scene_id0'][0],
                        frames[2]['scene_id0'][0],frames[3]['scene_id0'][0]
                    ])) == 1:
                scene_id = frames[0]['scene_id0'][0]
                if last_scene_id != scene_id:
                    last_scene_id = scene_id
                ...
                frame1_id = frames[1]['frame_id0'][0]
                frame2_id = frames[2]['frame_id0'][0]
                frame3_id = frames[3]['frame_id0'][0]


                # 兼容旋转数据集和普通数据集的键名
                box_key = 'box_ref' if 'box_ref' in frames[0] else 'box'
                box_normals_key = 'box_normals_ref' if 'box_normals_ref' in frames[0] else 'box_normals'
                
                box = torch.from_numpy(frames[0][box_key][0]).to(device)
                box_normals = torch.from_numpy(
                    frames[0][box_normals_key][0]).to(device)
                
                gt_pos1 = frames[1]['pos0'][0]
                gt_pos2 = frames[2]['pos0'][0]
                gt_pos3 = frames[3]['pos0'][0]

                gt_v1 = frames[1]['vel0'][0]
                gt_v2 = frames[2]['vel0'][0]
                gt_v3 = frames[3]['vel0'][0]
                ...
                inputs = (pr_pos2, pr_vel2, None, box, box_normals)
                ...
                fluid_errors.add_errors(scene_id, frame0_id, frame3_id,
                                        scale * pr_pos3.cpu().detach().numpy(),
                                        scale * gt_pos3) #第二帧误差     


                v_errors.add_errors(scene_id, frame0_id, frame1_id,
                                        scale * pr_vel1.cpu().detach().numpy(),
                                        scale * gt_v1) #第一帧误差
                v_errors.add_errors(scene_id, frame0_id, frame2_id,
                                        scale * pr_vel2.cpu().detach().numpy(),
                                        scale * gt_v2) #第二帧误差
                
                v_errors.add_errors(scene_id, frame0_id, frame3_id,
                                        scale * pr_vel3.cpu().detach().numpy(),
                                        scale * gt_v3) #第二帧误差  
                ...
                emd3 = wasserstein_distance(pr_pos3.cpu().detach().numpy().flatten(), gt_pos3.flatten())
                
                emd_distance1.append(emd1)
                emd_distance2.append(emd2)
                emd_distance3.append(emd3)

                v1 = wasserstein_distance(pr_vel1.cpu().detach().numpy().flatten(), gt_v1.flatten())
                v2 = wasserstein_distance(pr_vel2.cpu().detach().numpy().flatten(), gt_v2.flatten())
                v3 = wasserstein_distance(pr_vel3.cpu().detach().numpy().flatten(), gt_v3.flatten())
                
                v_distance1.append(v1)
                v_distance2.append(v2)
                v_distance3.append(v3)
    ...
    result = {}
    result['err_n1'] = np.nanmean(
        [v['mean'] for k, v in fluid_errors.errors.items() if k[1] + 1 == k[2]]) # first frame error
    result['err_n2'] = np.nanmean(
        [v['mean'] for k, v in fluid_errors.errors.items() if k[1] + 2 == k[2]]) # second frame error
    result['err_n3'] = np.nanmean( [v['mean'] for k, v in fluid_errors.errors.items() if k[1] + 3 == k[2]]) # third frame error    
    # for k, v in fluid_errors.errors.items():
    #     print(k)
    result['emd_n1'] = np.nanmean(emd_distance1)
    result['emd_n2'] = np.nanmean(emd_distance2)
    result['emd_n3'] = np.nanmean(emd_distance3)
    result['uncompress'] = np.nanmean(rou_array)
    print(result)
    print('done')


    result1 = {}
    result1['v_n1'] = np.nanmean(
        [v['mean'] for k, v in v_errors.errors.items() if k[1] + 1 == k[2]]) # first frame error
    result1['v_n2'] = np.nanmean(
        [v['mean'] for k, v in v_errors.errors.items() if k[1] + 2 == k[2]]) # second frame error
    result1['v_n3'] = np.nanmean( [v['mean'] for k, v in v_errors.errors.items() if k[1] + 3 == k[2]]) # third frame error    
    # for k, v in fluid_errors.errors.items():
    #     print(k)
    result1['vemd_n1'] = np.nanmean(v_distance1)
    result1['vemd_n2'] = np.nanmean(v_distance2)
    result1['vemd_n3'] = np.nanmean(v_distance3)
    result1['uncompress'] = np.nanmean(rou_array)
    print(result1)
    print('done')
...
def evaluate_whole_sequence_torch(model,
    ...
    for data in val_dataset:
        ...
        if frame_id > 0 and frame_id % skip == 0:
            ...
            fluid_errors.add_errors(scene_id,
                                    ...
                                    compute_gt2pred_distance=True)
            gt_v=data['vel0'][0]
            v_errors.add_errors(scene_id,
                                    0,
                                    frame_id,
                                    scale * pr_vel.detach().cpu().numpy(),
                                    scale * gt_v,
                                    compute_gt2pred_distance=True)
    ...
    result1 = {}
    result1['whole_seq_verr'] = np.mean([
        v['gt2pred_mean']
        for k, v in v_errors.errors.items()
        if 'gt2pred_mean' in v
    ])
...
def eval_checkpoint(checkpoint_path, val_files, fluid_errors, options, cfg):
    # checkpoint_path contains the trained model, val_files contains test dataset
    ...
    if checkpoint_path.endswith('.index'):
        ...
        if 'model' in checkpoint:
            model.load_state_dict(checkpoint['model']) # load trained model
        else:
...
def main():
    ...
    parser.add_argument("--trainscript",
                        type=str,
                        default=r"/media/data1/code/AAAI-simple/scripts/train_network_torch.py",
                        # required=True,
                        help="The python training script.")
    parser.add_argument("--cfg",
                        type=str,
                        default=r"/media/data1/code/AAAI-simple/scripts/default.yaml",
                        # required=True,
    ...
    if args.weights is not None:
        ...
        ]) # sort by iteration number in checkpoint filenames
        ...
        if os.path.isfile(output_path):
            print('Printing previously computed results for :', checkpoint,
                  output_path) # if the evaluation folder has the same name (same model iterations), print the previous results directly
            fluid_errors = FluidErrors()
            fluid_errors.load(output_path)
        else:
            print('evaluating :', checkpoint)
            fluid_errors = FluidErrors()
            eval_checkpoint(checkpoint[1], val_files, fluid_errors, args, cfg) # first evaluation
            fluid_errors.save(output_path)
