...
def get_simulation_params(path):
    ...
    # sequence
    seq_dir = parts[-1]
    seq_type = 'A' if seq_dir == 'constant_acceleration' else 'C'
    # dynamic model
    ...
    print(f"解析参数: dynamic={dynamic_model}, time={time}, roll={roll}, pitch={pitch}, fluid={fluid}, type={seq_type}")
    return time, roll, pitch, fluid, seq_type, dynamic_model
...
def process_single_dir(output_path, splits, sim_dir, num_threads=8):
    ...
    try:
        # 获取仿真参数 (新增 dynamic_model)
        time, roll, pitch, fluid, seq_type, dynamic_model = get_simulation_params(sim_dir)
        
        # 更新前缀，加入动态模型避免冲突
        prefix_name = f"sim42_D{dynamic_model}_T{time}_R{roll}_P{pitch}_{fluid}_{seq_type}"
        
        # 添加sim_0042子目录
        sim_dir = sim_dir / 'sim_0042'
...
def batch_compress_dataset(dataset_dir, output_dir, splits=16):
    ...
    # 获取所有需要处理的目录
    sim_dirs = list(dataset_path.rglob('constant_*'))
