...
from open3d.ml.torch.python.layers.convolutions import ContinuousConv
from torch import classes
from open3d.ml.torch.python.layers.neighbor_search import FixedRadiusSearch, RadiusSearch
import torch
...
class ContinuousConv(torch.nn.Module):
    def __init__(
        ...
        sh = np.array(self.kernel_size, dtype="int32")  # [6,6,6]
        assert sh[1] % 2 == 0
        sh[1] = np.ceil(sh[1] / 2)
        kernel_shape = [*sh, self.in_channels, self.filters]
    ...
    def forward(self,
        ...
        kernel = torch.concat([torch.flip(-self.kernel, [0, 1, 2]), self.kernel],
                              axis=1)
